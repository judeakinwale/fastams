...
def delete_invalid_attendance_history(attendance_history: schemas.AttendanceHistory):
    deleted_att_his = utils.mongo_res(
        models.AttendanceHistory.find_one_and_delete(
            {"_id": ObjectId(attendance_history["_id"])}
        )
    )
...
def get_detailed_attendance_history(attendance_history):
    ...
    user = utils.mongo_res(
        models.User.find_one({"_id": ObjectId(attendance_history["user_id"])})
    )
    ...
    location = utils.mongo_res(
        models.Location.find_one({"_id": ObjectId(attendance_history["location_id"])})
    )
...
def check_valid_user_for_facial_recognition(email: str):
    ...
    if not user["image"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No profile image found. Contact the admin to add an image to your profile.",
        )

    if not user["face_encoding"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No face found in profile image. Contact the admin to update your profile image.",
        )

    return user


def check_face_using_facial_recognition(
    user: schemas.User, file: UploadFile = File(...)
):
    ...
    if not image_matches:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Matching face not found. Try Again.",
        )
...
def save_attendance_image_file(
    first_name: str = "anon",
    email: str = "anon@test.com",
    file: UploadFile = File(None),
    is_qr_code: bool = True,
    folder_name: str = "attendance_images",
):
...
def get_user_and_email_from_qr_code(
    content: str | None = None, file: UploadFile = File(None)
):
    ...
    if not (content or file):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No QR Code provided. Try again.",
        )
    ...
    if not content:
        ...
        if not utils.validate_email(qr_code_content):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Invalid QR Code. Try again.",
            )
...
def check_recent_attendance_history(email: str, is_sign_out: bool = False):
    ...
    attendance_history = utils.mongo_res(
        models.AttendanceHistory.find_one(
            {
                "email": email,
    ...
    if attendance_history and attendance_history["is_signed_out"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"You have already clocked out",
        )
    ...
    if is_sign_out:
        ...
        if not attendance_history:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No attendance_history with this email: {email} found",
            )
    ...
    if attendance_history:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"You have signed in with this email: {email}. Sign out instead",
        )
...
def check_location(
    ...
    if use_location:
        if not location_id:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No location found for your account. Kindly contact the admin.",
            )

        location = utils.mongo_res(
            models.Location.find_one({"_id": ObjectId(location_id)})
        )
        if not location:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No location for user with this email: {email} found. Kindly contact the admin.",
            )

        if (not (long or lat)) or (long == "undefined" or lat == "undefined"):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"{attendance_type} location not provided!",
            )

        is_location_match = utils.check_matching_location(
            location, float(long), float(lat)
        )
        if not is_location_match:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"You are not allowed to {attendance_type} from this location",
            )
...
@router.post(
    ...
    response_model=schemas.AttendanceHistoryResponseWithUser,
    summary="Attendance Sign In",
)
...
def create_attendance_history(
    ...
    if not use_facial_recognition:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Facial Recognition Clockin and Clockout is disabled. Kindly contact the admin.",
        )
    ...
    relative_image_path = save_attendance_image_file(
        user["first_name"], email, file, False
    )
    ...
    attendance_history = get_detailed_attendance_history(
        models.AttendanceHistory.find_one({"_id": ObjectId(created_id)})
    )
...
@router.patch(
    "/sign-out",
    response_model=schemas.AttendanceHistoryResponseWithUser,
    summary="Attendance Sign Out",
)
# @router.post('/sign-out')
def update_attendance_history(
    email: str,
    file: UploadFile = File(...),
    long: str | None = None,
    lat: str | None = None,
    ...
    attendance_history = get_detailed_attendance_history(
        models.AttendanceHistory.find_one_and_update(
            {"_id": ObjectId(attendance_history["id"])},
            {"$set": payload},
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    response_model=schemas.AttendanceHistoryResponseWithUser,
    summary="Attendance Sign In with QR",
)
def create_attendance_history(
...
@router.post(
    "/sign-out-qr",
    response_model=schemas.AttendanceHistoryResponseWithUser,
    summary="Attendance Sign Out with QR",
)
...
def get_attendance_histories(
    ...
    attendance_history = [
        get_detailed_attendance_history(attendance)
        for attendance in models.AttendanceHistory.find(filter)
        .limit(limit)
        .skip(skip)
        .sort("created_at", -1)
    ]
...
@router.get(
    "/{attendance_history_id}", response_model=schemas.AttendanceHistoryResponseWithUser
)
# @router.get('/{attendance_history_id}')
def get_attendance_history(attendance_history_id: str):
    attendance_history = get_detailed_attendance_history(
        models.AttendanceHistory.find_one({"_id": ObjectId(attendance_history_id)})
    )
    if not attendance_history:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No attendance_history with this id: {attendance_history_id} found",
        )
...
def update_attendance_history(
    attendance_history_id: str, payload: schemas.UpdateAttendanceHistory
):
    payload = payload.dict(exclude_unset=True)
    payload.update(
        {
    ...
    attendance_history = get_detailed_attendance_history(
        models.AttendanceHistory.find_one_and_update(
            {"_id": ObjectId(attendance_history_id)},
            {"$set": payload},
            return_document=ReturnDocument.AFTER,
        )
    )
