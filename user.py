...
def get_detailed_user(user):
    ...
    try:
        attendance_history = [
            utils.mongo_res(attendance)
            for attendance in models.AttendanceHistory.find({"email": user["email"]})
        ]
        user["attendance_history"] = attendance_history

        location = utils.mongo_res(
            models.Location.find_one({"_id": user["location_id"]})
        )
...
def update_payload_with_password_hash(
    payload: schemas.CreateUser, generate_pass: bool = False
) -> schemas.CreateUser:
...
def save_user_image_file(
    first_name: str, email: str, file: UploadFile | None = File(None)
) -> str:
    # generate unique name for image
    random_chars = utils.random_string(
        5
    )  # for generating random strings for image and qr code
    file_extension = utils.get_file_extension(file.filename)
    relative_image_path = (
        f"./training_images/{first_name}-{email}-{random_chars}{file_extension}"
    )
...
def send_account_creation_email(
    email: str, first_name: str, image_path: str, is_admin=False
):
    reciepients = [email]
...
def user_factory(
    payload: schemas.CreateUser, file: UploadFile | None = File(None), is_admin=False
...
def get_users(limit: int = 1000000000000, page: int = 1, search: str = ""):
    ...
    users = [
        ...
        for user in models.User.find(filter)
        .limit(limit)
        .skip(skip)
        .sort("created_at", -1)
    ]
...
def get_absent_users(
    ...
    def check_date_attendance(
        attendance_history: List[schemas.AttendanceHistory] = [],
    ...
    users = [
        get_detailed_user(user)
        for user in models.User.find(filter).limit(limit).skip(skip)
    ]
...
def create_user(
    ...
    if use_location and not payload["location_id"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"The location_id is required"
        )
...
def delete_user(user_id: str):
    ...
    related_attendance_history = models.AttendanceHistory.delete_many(
        {"user_id": user_id}
    )
